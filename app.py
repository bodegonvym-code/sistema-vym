...
# ============================================
# SISTEMA DE USUARIOS CON LOCALSTORAGE Y PERMISOS
# ============================================
...
with st.sidebar:
    ...
    if not st.session_state.usuario_actual:
        with st.expander("🔐 Acceso al sistema", expanded=True):
            usuarios_activos = []
            try:
                res = db.table("usuarios").select("usuario").eq("activo", True).execute()
                if res.data:
                    usuarios_activos = [u['usuario'] for u in res.data]
            except:
                pass
        ...
        with st.container(border=True):
            ...
            try:
                ...
                if turno_info.data and turno_info.data[0].get('tasa_divisas') is not None:
                    tasa_divisas_actual = float(turno_info.data[0]['tasa_divisas'])
                else:
...
# ============================================
# MÓDULO 2: PUNTO DE VENTA (CON POPOVER BUSCADOR POR NOMBRE - VERSIÓN ANTERIOR)
# ============================================
    ...
    # ============================================
    # BUSCADOR POR NOMBRE EN POPOVER (VERSIÓN ANTERIOR)
    # ============================================
    with st.popover("🔍 Buscar por nombre", use_container_width=True):
        ...
        busqueda = st.text_input("", key="buscar_nombre_popover", placeholder="Ej: Harina, Aceite...", label_visibility="collapsed")
